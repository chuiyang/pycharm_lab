...
# self pred new data
new_name = ['new_total']
new_file = pd.read_csv('../../gpu_output/qm9_ens.csv', index_col=None).values
new_pred = new_file[:, 1]
new_legend = ['new_e', 'new_a', 'new_t']
new_e = new_file[:, 3]
new_a = new_file[:, 2]
new_t = new_file[:, 2] + new_file[:, 3]
for new_file_uncertainty in new_e, new_a, new_t:
    new_sort_file = np.vstack((smiles, true_value, new_pred, new_file_uncertainty)).T
    new_sort_file = new_sort_file[np.argsort(new_sort_file[:, 3])]
    new = []
    for j in range(100):
        percent = 100 - j  # 100, 99, 98, 97.... 1
        amount = n_mols * (percent / 100)
        y_true = new_sort_file[:int(amount), 1]
        y_pred = new_sort_file[:int(amount), 2]
        percent_mae = mean_absolute_error(y_true, y_pred)
        new.append(percent_mae)
    plt.plot(range(100), new)


plt.legend(['oracle', 'epi', 'ale', 'total'] + new_legend, loc='lower left')
