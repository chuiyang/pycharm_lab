...
# test_pred
molecules = pd.read_csv(f'./saved_models/pred_output/{place}/train_pred.csv', index_col=None).values[:, :]
smiles = molecules[:, 0]
...
# test_full
true_value = pd.read_csv(f'./saved_models/pred_output/{place}/train_full.csv', index_col=None).values[:, 1]

# grad_a
gradient = f'./saved_models/pred_output/{place}/autograd_a_std_mm.csv'  # grad_a_smaller
avg_grad = pd.read_csv(gradient, index_col=None).values[:, 6]
max_grad = pd.read_csv(gradient, index_col=None).values[:, 7]
...
# plt.legend(['oracle', 'max_grad*ale', 'avg_grad*ale', 'ale', 'epi'], loc='lower left')
plt.legend(['oracle', 'max_grad', 'avg_grad', 'ale', 'epi'], loc='lower left')
# plt.legend(['oracle', 'max_grad_std', 'avg_grad_std', 'ale', 'epi'], loc='lower left')
