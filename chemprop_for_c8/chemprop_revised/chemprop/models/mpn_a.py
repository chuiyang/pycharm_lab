...
import numpy as np
from chemprop.features.featurization_a import mol2graph_a, BatchMolGraph_a, get_atom_fdim, get_bond_fdim
...
class MPN_a(nn.Module):
    ...
    def forward(self,
                batch: Union[List[str], BatchMolGraph_a],
                features_batch: List[np.ndarray] = None) -> torch.FloatTensor:
        """
        ...
        if not self.graph_input:  # if features only, batch won't even be used
            batch = mol2graph_a(batch, self.args)
