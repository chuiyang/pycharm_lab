...
import numpy as np
from chemprop.features.featurization_autograd_a import mol2graph_autograd_a, BatchMolGraph_autograd_a, get_atom_fdim, get_bond_fdim
...
from .utils_a import f_bonds_change
import pickle


class MPNEncoder_a(nn.Module):
    ...
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int):
        ...
        """
        super(MPNEncoder_a, self).__init__()
    ...
    def forward(self,
                mol_graph: BatchMolGraph_autograd_a,
        ...
        mol_graph = mol2graph_autograd_a(mol_graph, self.args)
        f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, b_scope, smiles_batch = mol_graph.get_components()
        # f_bonds = f_bonds_change(f_bonds, b_scope[0])
        ...
        for depth in range(self.depth - 1):  # GCNN
            ...
            if self.atom_messages:
                ...
                nei_f_bonds = index_select_ND(features_batch, a2b)  # num_atoms x max_num_bonds x bond_fdim
...
class MPN_a(nn.Module):
    ...
    def __init__(self,
        ...
        self.graph_input = graph_input  # false
        self.encoder = MPNEncoder_a(self.args, self.atom_fdim, self.bond_fdim)

    def forward(self,
                batch: Union[List[str], BatchMolGraph_autograd_a],
                features_batch: List[np.ndarray] = None) -> torch.FloatTensor:
        """
        ...
        #     # print('batch', batch)
        #     # print(self.args)
        #     batch = mol2graph_autograd_a(batch, self.args)
        # print('batch', batch)
        # print('features_batch', features_batch)
        output = self.encoder.forward(batch, features_batch)  # batch -> smile ; feature_batch -> bond_vec # batch is molecular graph, feature batch none
