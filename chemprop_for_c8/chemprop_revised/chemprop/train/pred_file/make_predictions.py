...
def make_predictions(args: Namespace, smiles: List[str] = None) -> List[Optional[List[float]]]:
    ...
    for index, checkpoint_path in enumerate(tqdm(args.checkpoint_paths, total=len(args.checkpoint_paths))):
        ...
        model_preds, ale_uncs, epi_uncs, feature = predict(
            model=model,
            data=test_data,
        ...
        #########################
        every_path = args.preds_path.split('/')
        every_path.pop()
        every_path = '/'.join(every_path)
        # 存下該index model下的batch feature
        np.save(f'{every_path}/hidden_vector_{index}', np.array(feature))
        print('model', index, ':', np.array(feature).shape)
        # 把每個model的pred放到list 把每個model創一個header
        models_preds.append(model_preds)
        models_ales.append(ale_uncs)
        file_header.append(f'model_{index}') 
        #########################
        ...
        ##################################################
    print(file_header)
    models_preds = np.array(models_preds).reshape((len(models_preds), len(models_preds[0]))).T
    models_ales = np.array(models_ales).reshape((len(models_ales), len(models_ales[0]))).T
    print('models_preds shape:', models_preds.shape)
    pd.DataFrame(models_preds, index=full_data.smiles(), columns=file_header).to_csv(f'{every_path}/every_model_predictions.csv')
    pd.DataFrame(models_ales, index=full_data.smiles(), columns=file_header).to_csv(f'{every_path}/every_model_aleatorics.csv')
    ##################################################
