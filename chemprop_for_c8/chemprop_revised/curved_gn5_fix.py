...
if __name__ == '__main__':
    ...
    file_path = f'./saved_models/pred_output/curved_gn5_fix/test_fake_feature.csv'
    mol_fp = pd.read_csv(file_path).values[:, 0]
    true = mol_fp**3 - 20 * mol_fp
    pred = pd.read_csv('./saved_models/pred_output/curved_gn5_fix/test_fake_data_pred.csv').values[:, 1]

    grad_rmss = []
    grad_maxx = []
    # smiles = pd.read_csv('./saved_models/qm9_ens_woN/fold_0/qm9_N.csv').values[:, 0]
    smiles = pd.read_csv('./saved_models/pred_output/curved_gn5_fix/test_fake_data.csv').values[:, 0]
    for (i, smile, t) in zip(range(mol_fp.shape[0]), smiles, true):  # mol_fp.shape[0]

        mol_i = np.array(mol_fp[i])
